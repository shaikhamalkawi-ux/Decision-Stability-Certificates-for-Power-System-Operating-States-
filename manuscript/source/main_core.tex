...
The chronology, AC, GB, and Elexon claims are backed by machine-readable result tables rather than by figures alone. The released archive retains the pinned RTS-GMLC source commit, both minimum-up/down formulations, the exact July relaxation repair, the network witness and a new warm-started 600-s MIP log, the March LP-bound record, all 48 shared AC witness outputs, the independent PyPSA-GB source commit and solved-dispatch extracts, and raw Elexon responses with SHA-256 hashes. A clean-room script that imports no DSC-Grid analysis module reproduces 26 central numerical claims. This record is also why a negative solver event is not promoted to physical rejection: only transferable relaxation arguments or returned feasible witnesses support the corresponding claims.
...
The companion reproducibility archive contains source-lineage records, chronology formulations and result tables, AC projection code and per-hour outputs, cross-solver records, frozen PyPSA-GB dispatch extracts and parameter audits, raw Elexon responses and acquisition/analysis code, a clean-room claim audit, and SHA-256 manifests. RTS-GMLC and PyPSA-GB upstream sources are identified by pinned public repository commits; Elexon data are obtained from its public Insights API \cite{ElexonInsights}. A permanent repository DOI is pending author deposit.
