...
ROOT=PACKAGE_ROOT/'results'/'reproduction_ac_fixed'
...
opts={
    'ipopt.print_level':0,'print_time':0,'ipopt.max_iter':250,'ipopt.max_cpu_time':0.4,
...
results=[]; corrections=[]; previous={}; shared_hour_solution={}
for month in [3,7]:
    ...
    for h,rowd in d.iterrows():
        ...
        for mode in ['shared_continuous','fixed_commitment']:
            ...
            best=None; attempts=[]
            t0=time.time()
            seeds = initial_guesses(target_used,pmin,pmax,qmin,qmax,pdem,None)
            if mode=='fixed_commitment' and (month,int(h)) in shared_hour_solution:
                seeds=[('shared_hour',shared_hour_solution[(month,int(h))].copy())]
            ...
            if best is not None:
                ...
                if mode=='shared_continuous': shared_hour_solution[(month,int(h))]=xv.copy()
