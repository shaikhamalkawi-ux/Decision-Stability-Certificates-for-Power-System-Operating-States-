...
only released CSV/ZIP artifacts, reconstructs empirical L1 transport with a new
...
def read_static_csvs(package: Path) -> dict[int, pd.DataFrame]:
    archive = package / "upstream_lock" / "DSC_Grid_V4_ConvexSupportPolyhedralCertificate_FULL_PACKAGE.zip"
    prefix = "DSC_Grid_V4_ConvexSupportPolyhedralCertificate/upstream/V3/processed"
    out: dict[int, pd.DataFrame] = {}
    with zipfile.ZipFile(archive) as zf:
        for month in (1, 2, 3, 4, 5, 6, 7, 10):
            name = f"{prefix}/month_{month:02d}_first_week_dispatch.csv"
            out[month] = pd.read_csv(io.BytesIO(zf.read(name))).drop(columns="timestamp")
    return out
...
def audit_static(package: Path, rows: list[dict]) -> None:
    ...
    for month, (m_expected, w_expected) in STATIC_EXPECTED.items():
        ...
        add(rows, f"RTS month {month:02d} mean L1 (MW)", m_expected, mean_move,
            tol, "V4 upstream dispatch CSVs", "direct means; no project imports")
        add(rows, f"RTS month {month:02d} empirical W1 (MW)", w_expected, w1,
            tol, "V4 upstream dispatch CSVs", "Hungarian assignment on L1 costs")


def audit_gb(package: Path, rows: list[dict]) -> None:
    root = package / "results" / "gb_replication"
    ...
    metadata = pd.concat([
        pd.read_csv(root / "january_generator_metadata.csv"),
        pd.read_csv(root / "july_generator_metadata.csv"),
    ], ignore_index=True).drop_duplicates("name", keep="last")
    ...
    for claim, (expected, got) in values.items():
        add(rows, claim, expected, got, 1e-7, "released PyPSA-GB dispatch and metadata CSVs",
            "independent filtering, L1 assignment, and clock-hour grouping")


def audit_repairs_and_ac(package: Path, rows: list[dict]) -> None:
    chronology = package / "results" / "chronology" / "native_minud_repair_month_07.csv"
    ...
    network = package / "results" / "network_repair" / "network_repair_mean.csv"
    ...
    ac = pd.read_csv(package / "results" / "ac_shared" / "ac_projection_results.csv")
