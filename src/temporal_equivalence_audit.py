...
def run_twins(args):
    """Separately frozen four-case extension using existing permutation files."""
    model = load_model(args.source_v3)
    groups, members, inventory = make_groups(model, args.source_v3)
    original = ROOT / "results/temporal_information/equivalence"
    original_inventory = json.loads((original/"group_inventory.json").read_text())
    assert inventory == original_inventory, "The extension must retain exactly the original groups"
    original_freeze = json.loads((original/"pre_run_freeze.json").read_text())
    assert digest(original/"frozen_run_script.py") == original_freeze["script_sha256"]
    assert digest(original/"frozen_protocol.md") == original_freeze["protocol_sha256"]
    assert digest(original/"group_inventory.json") == original_freeze["group_inventory_sha256"]
    ...
    base = case_inputs(model, args.source_v3, 7)
    columns = model.dec["GEN UID"].tolist()
    ti = np.flatnonzero(model.thermal.to_numpy(bool))
    thermal_columns = model.dec.iloc[ti]["GEN UID"].tolist()
    ...
    u = pd.read_csv(u_path)[thermal_columns].to_numpy(float)
    assert p.shape == base["target"].shape and u.shape == (168,len(ti))
    base["target"] = p
    y, z = np.zeros_like(u), np.zeros_like(u)
    y[1:], z[1:] = np.maximum(0,np.diff(u,axis=0)), np.maximum(0,-np.diff(u,axis=0))
    positive = check_witness(model,base,groups,p,u,y,z)
    assert positive["pass"], positive
    control = args.output/"identity"
    control.mkdir(exist_ok=True)
    for name, values, labels in (("dispatch",p,columns),("commitment",u,thermal_columns),("startup",y,thermal_columns),("shutdown",z,thermal_columns)):
        frame = pd.DataFrame(values,columns=labels)
        frame.insert(0,"timestamp",base["timestamps"])
        frame.to_csv(control/(name+".csv"),index=False)
    reread = [pd.read_csv(control/(name+".csv"))[labels].to_numpy(float)
        for name,labels in (("dispatch",columns),("commitment",thermal_columns),("startup",thermal_columns),("shutdown",thermal_columns))]
    serialized = check_witness(model,base,groups,*reread)
    assert serialized["pass"], serialized
    write_json(control/"witness_verification.json", {"verdict":"ADMITTED_GROUPED_WITNESS", "direct":positive,
        "serialized":serialized,"source_dispatch_sha256":digest(p_path),"source_commitment_sha256":digest(u_path),
        "target":"repaired July network witness means; no optimization for identity"})
    pd.DataFrame([{"group_id":g["group_id"],"members":";".join(g["members"]),
        "pooled_target_mean_MW":float(p[:,g["indices"]].sum(axis=1).mean()),
        "pooled_target_energy_MWh":float(p[:,g["indices"]].sum())} for g in groups]).to_csv(args.output/"shared_group_target.csv",index=False)
    twins = ROOT/"results/temporal_information/twins"
    shared = pd.read_csv(twins/"shared_target.csv").set_index("uid").loc[columns]
    assert np.allclose(shared["target_mean_MW"].to_numpy(),p.mean(axis=0),atol=1e-10,rtol=0)
    pending=[]
    source_paths=[]
    for seed in range(26092600,26092604):
        folder=twins/f"seed_{seed}"
        permutation_path=folder/"permutation.csv"
        permutation=pd.read_csv(permutation_path)
        assert np.array_equal(permutation["new_hour_0based"],np.arange(168))
        order=permutation["source_hour_0based"].to_numpy(int)
        ...
        assert np.array_equal(permutation["source_native_row"],np.asarray(base["native_rows"])[order])
        case={**base,"pmin":base["pmin"][order],"pmax":base["pmax"][order],"demand":base["demand"][order],
            "native_rows":np.asarray(base["native_rows"])[order].tolist()}
        for group in groups:
            js=group["indices"]
            assert np.all(case["pmin"][:,js]==case["pmin"][:,js[0],None])
            assert np.all(case["pmax"][:,js]==case["pmax"][:,js[0],None])
        comparison_path=folder/"full_solver_crosscheck.json"
        comparison=json.loads(comparison_path.read_text())
        output=args.output/f"seed_{seed}"
        output.mkdir(exist_ok=True)
        permutation.to_csv(output/"permutation.csv",index=False)
        pending.append((seed,case,comparison,output))
        source_paths.extend([permutation_path,comparison_path])
    paths=[Path(__file__),PROTOCOL,original/"group_inventory.json",original/"pre_run_freeze.json",
        p_path,u_path,twins/"shared_target.csv",args.source_v3/"code/dscgrid_model.py",
        *sorted((args.source_v3/"raw").rglob("*.csv")),*source_paths]
    manifest=[{"path":str(path),"bytes":path.stat().st_size,"sha256":digest(path)} for path in dict.fromkeys(paths)]
    pd.DataFrame(manifest).to_csv(args.output/"input_manifest.csv",index=False)
    write_json(args.output/"pre_run_freeze.json",{"frozen_before_grouped_twin_solves_utc":datetime.now(timezone.utc).isoformat(),
        "extension":"post-pilot separate four-case diagnostic", "protocol_sha256":digest(PROTOCOL),
        "script_sha256":digest(Path(__file__)),"group_inventory_sha256":digest(args.output/"group_inventory.json"),
        "same_group_inventory_as_archived_weeks":True,"seeds":[item[0] for item in pending],"seconds_per_case":60,
        "positive_identity_witness_verified":True,"target":"repaired July witness group means"})
    print(json.dumps({"frozen_twins":True,"groups":len(groups),"positive_identity_verified":True}),flush=True)
    results=[]
    for seed,case,comparison,output in pending:
        record=solve_grouped(model,case,groups,output,args.seconds)
        record.update({"seed":seed,"case":f"seed_{seed}","individual_twin_verdict":comparison["verdict"],
            "individual_twin_model_status":comparison["model_status"],"target":"repaired July witness group means"})
        write_json(output/"solver_result.json",record)
        ...
        print(json.dumps({key:record[key] for key in ("case","model_status","verdict","elapsed_s","individual_twin_verdict")}),flush=True)
...
def main():
    ...
    parser.add_argument("--twins",action="store_true",help="Run the separate post-pilot four-twin extension")
    parser.add_argument("--seconds",type=float,default=60.)
    args = parser.parse_args()
    if args.output is None:
        args.output=ROOT/"results/temporal_information"/("equivalence_twins" if args.twins else "equivalence")
    if args.seconds != 60.:
